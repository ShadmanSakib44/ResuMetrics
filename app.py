...
app.config['MONGO_URI']= 'mongodb+srv://shadmansakib:12345@cluster0.9mfuwd8.mongodb.net/ResumeRanking'
...
def signup():
    if request.method == 'POST':
        ...
        if status is not None:
            # If user created successfully, set a success flag in the session
...
def login_normal():
    if request.method == 'POST':
        ...
        user = IRS_USERS.find_one({"Email": email, "Password": password})
        ...
        if user:
            ...
            return render_template('index.html', errMsg='Invalid email or password. Please try again.')
